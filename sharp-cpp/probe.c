...
static void probe_predefined_file_line(void) {
    ...
    if (!strstr(r.text, "2")) BAD("__LINE__ should give the line number: %s", r.text);
    else OK("got: %s", r.text);
    cpp_result_free(&r);
}
...
static void probe_extremely_long_token(void) {
    ...
    char *src = (char *)cpp_xmalloc(10100);
    memset(src, 'a', 10000);
    src[10000] = '\n';
    src[10001] = '\0';
    CppResult r = run(src);
...
static void probe_at_in_string(void) {
    PROBE("'@' inside a string literal is never tokenized as @-intrinsic");
    CppResult r = run("\"@foo\"\n");
