 ...
 *   CppResult res = cpp_run(ctx, "input.c");
...
typedef enum {
    ...
    CPPT_AT_INTRINSIC,   /* @ident  — Sharp compile-time intrinsic token.
                          ...
                          * "@has_operator".  Never macro-expanded;
...
/* -------------------------------------------------------------------------
 * Target-triple macro injection
 * ---------------------------------------------------------------------- */

/**
 * install_zig_target_macros — install builtin macros from generated data.
 * Fast-path replacement for cpp_probe_zig_macros.  Does NOT spawn zig.
 */
void install_zig_target_macros(CppCtx *ctx, const char *target);

/**
 * cpp_probe_zig_macros — Query zig cc for predefined macros (spawns zig cc -E -dM).
 * This is now a DEPRECATED fallback.  Prefer install_zig_target_macros.
 */
int  cpp_probe_zig_macros(CppCtx *ctx, const char *target);
 ...
 */
int  cpp_probe_host_macros(CppCtx *ctx);

/**
 * Backwards-compatible shim.  Probes macros for the host target
 * using zig cc -E.  Returns 0 on success, -1 if zig cc is unavailable.
 */
static inline int
cpp_install_target_macros(CppCtx *ctx, const char *target)
{
    return cpp_probe_zig_macros(ctx, target);
}
