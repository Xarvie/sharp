...
/* =========================================================================
 * Macro save stack — #pragma push_macro / pop_macro (GCC/MSVC extension)
 * ====================================================================== */
typedef struct MacroSaveFrame {
    const char             *name;
    bool                    was_defined;
    bool                    is_func;
    bool                    is_variadic;
...
struct MacroTable {
    ...
    /* #pragma push_macro / pop_macro save stack */
    MacroSaveFrame *save_stack;
};
...
MacroDef *macro_parse_define(const TokList *line_toks,
     ...
     * function-like macro.  leading_ws distinguishes:
    ...
    if (cur && cur->tok.kind == CPPT_PUNCT &&
        ...
        !(cur->tok.leading_ws && cur->tok.leading_ws[0])) {
        def->is_func = true;
...
static PPTok stringify(const TokList *arg, CppLoc loc) {
    ...
    for (TokNode *n = arg->head; n; n = n->next) {
        ...
        if (!first && n->tok.leading_ws && n->tok.leading_ws[0] &&
...
static PPTok token_paste(const PPTok *lhs, const PPTok *rhs,
     ...
     * leading_ws.  The paste result inherits the LHS spacing. */
    result.leading_ws = lhs->leading_ws;
    return result;
}
    ...
    while (n) {
        ...
        if (t->kind == CPPT_IDENT && def->is_func) {
            ...
            if (idx >= 0 && idx < max_idx) {
                ...
                /* Check for ## on the LEFT side (prev non-space token) */
                bool paste_left = false;
                if (result.tail && result.tail->tok.kind == CPPT_PUNCT &&
                    strcmp(pptok_spell(&result.tail->tok), "##") == 0) {
                    paste_left = true;
                }
...
static void expand_list(TokList *input, MacroTable *mt,
    ...
    for (TokNode *n = input->head; n; n = n->next) {
         ...
         * _Null_unspecified) are used extensively in macOS / iOS system
         * headers.  Clang treats these as reserved nullability annotation
         * keywords that are NOT subject to macro expansion, even when
         * system headers #define them as empty macros (which zig's libc
         * headers do).  If we expanded them, they would disappear from
         * the output, breaking token-identical comparison with clang.
         * Pass them through verbatim regardless of any #define.        */
        ...
        if (!def) {
            /* defined operator in macro body: its argument must NOT be
             * macro-expanded per GCC semantics.  Scan ahead for
             * defined ( IDENT ) or defined IDENT and mark the
             * identifier hidden so post-expansion resolve_all_defined
             * can check whether it is a real macro. */
            if (strcmp(name, "defined") == 0) {
                ...
                if (nx && nx->tok.kind == CPPT_PUNCT &&
                    ...
                    if (nx && nx->tok.kind == CPPT_IDENT)
                        nx->tok.hide = true;
                } else if (nx && nx->tok.kind == CPPT_IDENT) {
                    nx->tok.hide = true;
                }
            }
...
void macro_push_state(MacroTable *t, const char *name) {
    MacroSaveFrame *f = cpp_xmalloc(sizeof *f);
    f->name    = name;
    f->next    = t->save_stack;
    t->save_stack = f;
    ...
    if (def) {
        f->was_defined = true;
        f->is_func     = def->is_func;
        f->is_variadic = def->is_variadic;
        f->nparams     = def->nparams;
        f->def_loc     = def->def_loc;
        f->param_names = NULL;
        if (def->nparams > 0 && def->param_names) {
            f->param_names = cpp_xmalloc((size_t)def->nparams * sizeof(char *));
            for (int i = 0; i < def->nparams; i++)
                f->param_names[i] = cpp_xstrdup(def->param_names[i]);
        }
        f->body = (TokList){0};
        for (TokNode *n = def->body.head; n; n = n->next)
            tl_append_copy(&f->body, &n->tok);
    } else {
        f->was_defined = false;
        f->is_func     = false;
        f->is_variadic = false;
...
void macro_pop_state(MacroTable *t, const char *name, CppDiagArr *diags) {
    MacroSaveFrame **pp = &t->save_stack;
    ...
    *pp = f->next;
    ...
    if (f->was_defined) {
        MacroDef *def = cpp_xmalloc(sizeof *def);
        def->name       = name;
        def->is_func    = f->is_func;
        def->is_variadic = f->is_variadic;
        def->nparams    = f->nparams;
        def->def_loc    = f->def_loc;
        def->next       = NULL;
        def->param_names = NULL;
        if (f->nparams > 0 && f->param_names) {
            def->param_names = cpp_xmalloc((size_t)f->nparams * sizeof(char *));
            for (int i = 0; i < f->nparams; i++)
                def->param_names[i] = cpp_xstrdup(f->param_names[i]);
        }
        def->body = (TokList){0};
    ...
    if (f->param_names) {
        for (int i = 0; i < f->nparams; i++) free(f->param_names[i]);
        free(f->param_names);
    }
    tl_free(&f->body);
    free(f);
...
void macro_free_state_stack(MacroTable *t) {
    while (t->save_stack) {
        MacroSaveFrame *f = t->save_stack;
        t->save_stack = f->next;
        if (f->param_names) {
            for (int i = 0; i < f->nparams; i++) free(f->param_names[i]);
            free(f->param_names);
