...
CppCtx *cpp_ctx_new(void) {
    ...
    ctx->phase6_concat     = false;
    /* Phase R12: trigraph translation defaults OFF to match `gcc -E`'s
     * default (gcc only warns about trigraphs unless -trigraphs is set). */
    ctx->trigraphs         = false;
    return ctx;
}
...
/* Phase R15: function-like macro variant.  name_with_params looks like
 * "__INT64_C(c)" — the parameter list is part of the name string.
 * macro_define_object → macro_parse_define already handles this format,
 * so the storage path is identical to cpp_define.                     */
void cpp_define_func(CppCtx *ctx, const char *name_with_params, const char *body) {
    cpp_define(ctx, name_with_params, body);
}
...
void cpp_set_lang_std(CppCtx *ctx, long std)        { ctx->lang_std          = std;  }
void cpp_set_phase6_concat(CppCtx *ctx, bool on)    { ctx->phase6_concat     = on;   }
void cpp_set_trigraphs(CppCtx *ctx, bool on)        { ctx->trigraphs         = on;   }
...
static char *phase6_apply_text(const char *text, size_t len, size_t *out_len) {
    ...
    while (p < end) {
        ...
        StrBuf body = {0};
        bool body_ok = p6_read_body(&p, end, &body);
        /* Phase R4: if p6_read_body returns false it backed up *p by one,
         * leaving us pointing at the offending `"`.  In that situation
         * the body is malformed (unterminated across a newline) — the
         * lexer has already emitted "unterminated string literal".  We
         * must NOT fall into the adjacency-collect loop because that
         * would re-enter p6_read_body at the same `"` and spin forever
         * (this is mcpp's u_1_8.c hang).  Just emit the partial token
         * verbatim and advance past the `"`.                             */
        if (!body_ok) {
            sb_push_cstr(&out, prefix);
            sb_push_ch(&out, *p++);   /* the orphaned `"` */
            sb_free(&body);
            continue;
        }
        ...
        for (;;) {
            ...
            p = peek;
            if (!p6_read_body(&p, end, &body)) {
                /* Same hazard as above: the adjacent string is malformed.
                 * Stop adjacency collection so we don't loop on the
                 * same `"`.  Advance past the orphaned quote.            */
                if (p < end && *p == '"') p++;
                break;
            }
        }
...
static CppResult build_result(CppState *st, CppCtx *ctx, CppDiagArr *diags) {
    ...
    /* Phase R12: phase 6 is now opt-in.  By default (gcc/clang -E
     * compatibility) we just copy the raw output text into a fresh heap
     * buffer.  When enabled, phase6_apply_text scans for adjacent string
     * literal runs and merges them.  Either way, res.text and
     * res._raw_text are two SEPARATE heap allocations — cpp_result_free
     * frees both unconditionally, so we must never alias them.          */
    const char *raw_text = cpp_state_text(st);
    size_t      raw_len  = cpp_state_text_len(st);

    char  *ph6_text;
    size_t ph6_len;
    if (ctx->phase6_concat) {
        ph6_text = phase6_apply_text(raw_text, raw_len, &ph6_len);
    } else {
        ph6_text = (char *)cpp_xmalloc(raw_len + 1);
        if (raw_len) memcpy(ph6_text, raw_text, raw_len);
        ph6_text[raw_len] = '\0';
        ph6_len = raw_len;
    }
...
CppResult cpp_run(CppCtx *ctx, const char *filename) {
    CppDiagArr diags = {0};
    CppState *st = cpp_state_new(ctx, &diags);
    cpp_state_run_file(st, filename);
...
CppResult cpp_run_buf(CppCtx *ctx, const char *buf, size_t len,
    ...
    CppDiagArr diags = {0};
    CppState *st = cpp_state_new(ctx, &diags);
    cpp_state_run_buf(st, buf, len, filename);
    CppResult res = build_result(st, ctx, &diags);
    cpp_state_free(st);
    return res;
}
...
static const char *tok_kind_names[CPPT_COUNT] = {
    ...
    [CPPT_AT_INTRINSIC] = "at-intrinsic",
};
