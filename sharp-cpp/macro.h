...
/**
 * #pragma push_macro("NAME") — save the current definition of NAME.
 */
void macro_push_state(MacroTable *t, const char *name);

/**
 * #pragma pop_macro("NAME") — restore the most recently saved state of NAME.
 */
void macro_pop_state(MacroTable *t, const char *name, CppDiagArr *diags);

/**
 * Free all saved macro states (called during macro_table_free).
 */
