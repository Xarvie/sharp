...
static const char *const kGccBuiltins[] = {
                    ...
                    /* Clang __is_target_* builtins used by Availability headers */
                    "__is_target_arch",
                    ...
                    /* _FORTIFY_SOURCE _chk builtins (Darwin / glibc) */
...
static const char *const kGccAttrs[] = {
                ...
                "indirect_return",
                /* Clang attributes used by macOS Availability headers */
                "availability",
                /* Darwin / macOS Clang attributes used by sys/cdefs.h */
                "swift_attr",
                "disable_tail_calls",
                "not_tail_called",
                "enum_extensibility",
...
/* -----------------------------------------------------------------
 * kGccExtensions — GCC/Clang extensions recognised by
 * __has_extension(name) and __has_feature(name).
 * These are features that clang/zig cc supports for C compilation.
 * ----------------------------------------------------------------- */
static const char *const kGccExtensions[] = {
    "gnu_asm",
    "gnu_asm_goto",
    ...
    "assume_nonnull",
    "cxx_fixed_enum",
    /* Clang features used by macOS Availability headers */
...
static Value eval_primary(EvalCtx *ec) {
     ...
     * All others return 0.  __has_builtin returns results from kGccBuiltins.
     * __has_feature / __has_extension use kGccExtensions for known-good
     * features (currently gnu_asm, which must pass to avoid false #if
     * condition evaluation differences with zig-cc/clang).
     */
    if (t->kind == CPPT_IDENT) {
        ...
        bool is_has_featext = (strcmp(idname, "__has_feature")      == 0 ||
                               strcmp(idname, "__has_extension")    == 0);
        bool is_has_cpp     = (strcmp(idname, "__has_c_attribute")  == 0 ||
                               strcmp(idname, "__has_cpp_attribute")== 0);
        bool is_has_other   = is_has_featext || is_has_cpp;
        if (is_has_attr || is_has_builtin || is_has_other) {
            ...
            if (is_has_cpp)     return vsigned(0);
            if (is_has_featext) return vbool(table_contains(kGccExtensions, argbuf));
     ...
     * If followed by '(' it's a call to an unknown compiler builtin
     * (e.g. __building_module(...)) — consume the parenthesized args. */
    if (t->kind == CPPT_IDENT) {
        ec_skip_ws(ec);
        if (ec_peek(ec) && ec_peek(ec)->kind == CPPT_PUNCT &&
