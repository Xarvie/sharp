...
typedef struct PPTok {
    ...
    bool        at_bol;
    const char *leading_ws;        /* interned whitespace string (may be "") */
} PPTok;
...
struct CppState {
    ...
    /* Deferred #if @... depth -------------------------------------------
     * When the preprocessor encounters a #if whose condition contains one or
     * more CPPT_AT_INTRINSIC tokens, it cannot evaluate it (the @ expression
     * may reference a generic type parameter).  Instead it enters "deferred
     * mode": the entire #if...#endif block (including the directive lines
     * and the body) is passed through verbatim to the
    ...
    bool         line_has_content;    /* true if current line produced non-ws content */
};
