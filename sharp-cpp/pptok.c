...
CppReader *reader_new_from_buf(const char *buf, size_t len,
                               ...
                               InternTable *interns,
                               CppDiagArr *diags) {
    /* Make a copy so we can modify it (raw string conversion). */
    char *copy = (char *)cpp_xmalloc(len + 1);
    memcpy(copy, buf, len);
    copy[len] = '\0';
...
PPTok reader_next_tok(CppReader *rd, bool keep_comments) {
    ...
    /* Skip horizontal whitespace, accumulate leading whitespace string.
    ...
    tok.leading_ws = intern_get(rd->interns, ws_buf.buf, ws_buf.len);
    sb_free(&ws_buf);
    rd->at_bol     = false;
