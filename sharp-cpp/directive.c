 ...
 *
 * The Sharp language has no special preprocessor directives beyond standard C.
 * Sharp-specific syntactic constructs (@intrinsics, generics, operators) are
 * handled in the tokenizer (CPPT_AT_INTRINSIC) and deferred-condition passthrough.
 */
...
/*
 * Linemarker emission — mirrors GCC / Clang output conventions.
 */

static void fix_token_ptrs(struct CppState *st, const char *old_buf);
...
static void emit_linemarker_ex(CppState *st, int line, const char *file,
    ...
    if (st->out_text.len > 0 && st->out_text.buf[st->out_text.len - 1] != '\n') {
        const char *_ob = st->out_text.buf;
        sb_push_ch(&st->out_text, '\n');
        fix_token_ptrs(st, _ob);
    }
 ...
 *
 * Phase 1: when ctx->keep_whitespace is true, a CPPT_SPACE token is also
 * pushed into the public token array immediately before this token if
 * t->leading_ws is non-empty.  The pptok layer folds runs of horizontal
 * whitespace into the next token's leading_ws string, so this is the
 * canonical point at which "preserved" whitespace materialises in the
 * public output.
 *
...
static void emit_tok_text(CppState *st, const PPTok *t) {
    ...
    if (t->leading_ws && t->leading_ws[0]) {
        size_t ws_start = st->out_text.len;
        PUSH_CSTR(st, t->leading_ws);
...
static void install_builtins(CppState *st) {
    MacroTable  *mt = st->macros;
    ...
    /* __STRICT_ANSI__: gcc defines this when any -std= flag is active
     * (e.g. -std=c11, -std=c99).  MinGW headers (stdlib.h, stdio.h,
     * etc.) use #ifndef __STRICT_ANSI__ to gate non-ANSI extensions.
     * Without it, sharp-cpp's token output includes declarations like
     * wtoll/lltoa that gcc -std=c11 omitted — producing a systemic
     * diff starting at the first such gate (Pattern B/C regression).   */
    install_builtin_num(mt, it, "__STRICT_ANSI__", 1);
    /* __STDC_UTF_16__ / __STDC_UTF_32__: required by C11 <uchar.h>/<stdatomic.h>.
     * If not defined, MinGW's stdatomic.h omits atomic_char16_t/atomic_char32_t
     * and some uchar.h paths diverge from gcc's token stream.              */
    install_builtin_num(mt, it, "__STDC_UTF_16__",  1);
    install_builtin_num(mt, it, "__STDC_UTF_32__",  1);
    ...
    install_builtin_str(mt, it, "__SHARP_VERSION__", "\"1.0\"");
...
static void process_include_file(CppState *st, const char *filename,
                                  bool is_sys_header);
static void process_buf(CppState *st, CppReader *rd);
...
static char *find_include_ex(CppState *st, const char *name, bool is_system,
    ...
    /* User include paths (-I dirs).
     * GCC/clang search -I directories for BOTH "..." and <...> includes.
     * We do the same: user_paths are always searched, regardless of is_system.
     * (The only difference between "" and <> is that "" also checks relative
     * to the including file's directory first, handled above.) */
    ...
    for (int pi = 0; pi < 2; pi++) {
        ...
        for (size_t i = 0; i < arr->len; i++) {
            ...
            if (skipping && strcmp(arr->data[i], skip_dir) == 0) {
                skipping = false;
                skip_dir_handled = true;
                continue;
            }
            if (skipping) continue;
            /* Once skip_dir has been handled, skip any further
             * occurrences of the same directory (can appear in both
             * user_paths and sys_paths). */
            if (skip_dir_handled && strcmp(arr->data[i], skip_dir) == 0)
                continue;
...
static void handle_include(CppState *st, TokList *line, CppLoc loc,
    ...
    st->include_depth--;
    /* Phase R12: returning to enclosing file — match gcc/clang behavior.
     * GCC emits two markers on include return:
     *   1) # <resume_line> "<file>" 3   — system header re-entry
     *   2) # <next_line>  "<file>" 2 3  — back to enclosing context
     * We emit a single re-entry marker with flag 3, matching GCC's
     * dominant pattern.  Flag 4 (extern-C) is omitted to match GCC.     */
    if (is_sys_header) {
        emit_linemarker_ex(st, loc.line, current_file, 3, 0, 0);
    }
    emit_linemarker_ex(st, loc.line + 1, current_file, 2,
                       is_sys_header ? 3 : 0, 0);
}
...
static TokList resolve_defined(MacroTable *mt, const TokList *in) {
    ...
    for (TokNode *n = in->head; n; n = n->next) {
        ...
        if (n->tok.kind == CPPT_IDENT &&
            ...
            /* defined() with empty content: the identifier between parens
             * was macro-expanded away.  Emit 0 and consume the closing
             * paren if present. */
            if (have_paren) {
                while (nx && nx->tok.kind == CPPT_SPACE) nx = nx->next;
                if (nx && nx->tok.kind == CPPT_PUNCT &&
...
static void handle_if(CppState *st, TokList *line, CppLoc loc) {
    ...
    /* Pre-step: substitute dynamic built-in macros __LINE__ and __FILE__
     * before macro expansion.  cpp_eval_if_expr treats unrecognised
     * identifiers as 0 (C99 §6.10.1p4), so without this substitution
     * `#if 1000 != __LINE__` would evaluate as `1000 != 0` = true even
     * after `#line 1000`. */
...
static void handle_line(CppState *st, CppReader *rd, TokList *line, CppLoc loc) {
     ...
     *
     * C99 §6.10.4: "the line number of the NEXT source line is changed to
     * the value of the digit sequence".  The newline at the end of this
     * directive line increments the reader's counter by one, so we must
     * pre-set to N-1 so that after the increment the NEXT line is N. */
    if (rd) {
...
static void emit_deferred_directive(CppState *st, const char *dname,
    ...
    for (TokNode *n = line->head; n; n = n->next) {
        ...
        if ((t->leading_ws && t->leading_ws[0]) || t->kind == CPPT_SPACE)
            PUSH_CH(st, ' ');
...
static void handle_pragma(CppState *st, TokList *line, CppLoc loc,
    ...
    /* Collect pragma text (GCC does NOT macro-expand pragma bodies).
     * Preserve original spacing via leading_ws.                      */
    bool pragma_first = true;
    for (TokNode *n = line->head; n; n = n->next) {
        if (n->tok.kind == CPPT_SPACE) {
            sb_push_ch(&sb, ' ');
        } else {
            if (!pragma_first && n->tok.leading_ws && n->tok.leading_ws[0])
                sb_push_ch(&sb, ' ');
            sb_push_cstr(&sb, pptok_spell(&n->tok));
        }
        pragma_first = false;
    ...
    /* #pragma push_macro / pop_macro: MSVC pragmas for saving/restoring
     * macro definitions.  We implement them here (not just suppress them)
     * because MinGW headers (minwindef.h, _push_BOOL.h) depend on them.  */
    {
        const char *body = sb.buf ? sb.buf : "";
        while (*body == ' ') body++;
        bool is_push = strncmp(body, "push_macro", 10) == 0 &&
                       (body[10] == '\0' || body[10] == ' ' || body[10] == '(');
        bool is_pop  = strncmp(body, "pop_macro", 9) == 0 &&
                       (body[9] == '\0' || body[9] == ' ' || body[9] == '(');
        if (is_push || is_pop) {
            const char *p = is_push ? body + 10 : body + 9;
            while (*p == ' ') p++;
            const char *mname_start = NULL;
            size_t mname_len = 0;
            if (*p == '(') {
                p++;
                while (*p == ' ') p++;
                if (*p == '"') {
                    ...
                    mname_start = p;
                    while (*p && *p != '"') p++;
                    mname_len = (size_t)(p - mname_start);
            ...
            if (mname_start && mname_len > 0) {
                char *name = cpp_xstrndup(mname_start, mname_len);
                const char *iname = intern_cstr(st->interns, name);
                free(name);
                if (is_push)
                    macro_push_state(st->macros, iname);
                else
                    macro_pop_state(st->macros, iname, st->diags);
            }
            sb_free(&sb);
            return;
        }
    ...
    /* #pragma region / #pragma endregion / #pragma mark: clang treats these
     * as IDE directives and suppresses them in -E output.  We do the same
     * so our token output matches clang's exactly.                       */
    {
        const char *body = sb.buf ? sb.buf : "";
        while (*body == ' ') body++;
        if ((strncmp(body, "region", 6) == 0 &&
             (body[6] == '\0' || body[6] == ' ')) ||
            (strncmp(body, "endregion", 9) == 0 &&
             (body[9] == '\0' || body[9] == ' ')) ||
            (strncmp(body, "mark", 4) == 0 &&
             (body[4] == '\0' || body[4] == ' ')) ||
            (strncmp(body, "GCC system_header", 17) == 0 &&
             (body[17] == '\0' || body[17] == ' '))) {
            sb_free(&sb);
            return;  /* suppress these pragmas */
        }
    }
...
static PPTok make_file_tok(CppState *st, CppLoc loc) {
    ...
    char buf[4096];
    size_t pos = 0;
    const char *s = loc.file ? loc.file : "";
    #ifndef _WIN32
    /* Use only the basename on POSIX to match zig cc behaviour. */
    const char *slash = s;
 ...
 * _Pragma("X") or __pragma(X) sequences into `#pragma X\n`.
 * Without this, _Pragma/__pragma tokens from macro expansions
 * pass through as raw tokens, diverging from clang -E output.  */
static void emit_expanded_list(CppState *st, TokList *tl, CppLoc inv_loc,
    ...
    for (TokNode *en = tl->head; en; en = en->next) {
        ...
        /* Detect __pragma ( expr ) in the expanded token stream */
        if (tok->kind == CPPT_IDENT &&
            strcmp(pptok_spell(tok), "__pragma") == 0) {
            TokNode *n = en->next;
            while (n && n->tok.kind == CPPT_SPACE) n = n->next;
            if (!n || n->tok.kind != CPPT_PUNCT ||
                strcmp(pptok_spell(&n->tok), "(") != 0) goto passthrough2;
            n = n->next;  /* skip '(' */
            ...
            while (n && depth > 0) {
                ...
                PUSH_CSTR(st, pptok_spell(&n->tok));
                n = n->next;
            }
            if (depth == 0) {
                PUSH_CH(st, '\n');
...
static void process_buf(CppState *st, CppReader *rd) {
    /* Sharp @-intrinsic token recognition is always enabled — the
     * CppReader constructor defaults sharp_mode=true.  Safe for .c/.h
     * files which never contain '@' at the start of an identifier.     */

    bool at_bol = true;
    st->line_has_content = false;
    TokList pending_replay = {0};

    for (;;) {
        PPTok t;
        if (pending_replay.head) {
            t = pending_replay.head->tok;
            TokNode *old = pending_replay.head;
            pending_replay.head = old->next;
            if (!pending_replay.head) pending_replay.tail = NULL;
            pending_replay.len--;
            free(old);
        } else {
            t = reader_next_tok(rd, st->keep_comments);
        }
        ...
        if (t.kind == CPPT_PUNCT &&
            ...
            /* -------------------------------------------------------
             * Deferred conditional block passthrough.
             *
        ...
        if (t.kind == CPPT_IDENT) {
            ...
            /* __pragma(expr) — MSVC keyword.  Convert to #pragma expr
             * in the output so it matches clang's -E output exactly.   */
            if (strcmp(name, "__pragma") == 0) {
                /* Skip whitespace to find '(' */
                PPTok nx = reader_next_tok(rd, false);
                while (nx.kind == CPPT_SPACE) {
                    pptok_free(&nx);
            ...
            /* Clang nullability annotations (_Nullable, _Nonnull,
             * _Null_unspecified) must NOT be expanded even if a system
             * header defines them as empty macros.  Clang treats these as
             * reserved keywords.  Pass them through verbatim.            */
            if (_macdef && (strcmp(name, "_Nullable") == 0 ||
            ...
            if (_macdef) {
                ...
                bool macro_had_leading_space = t.leading_ws && t.leading_ws[0];
                    ...
                    if (nx.kind == CPPT_PUNCT &&
                        ...
                        while (depth > 0) {
                            ...
                            if (at.kind == CPPT_PUNCT) {
                                ...
                                else if (strcmp(pptok_spell(&at), ")") == 0) {
                                    depth--;
                                    if (depth == 0) closing_loc = at.loc;
                                }
                            }
                        }
                        macro_inv_loc = closing_loc;
                    } else {
                ...
                if (macro_had_leading_space && expanded.head) {
                    expanded.head->tok.leading_ws = intern_get(st->interns, " ", 1);
                }
                 ...
                 * in the rescan input (they won't be read again by the main loop). */
                {
                    ...
                    if (_rescan_def && macro_def_is_func(_rescan_def)) {
                        ...
                        if (first.kind == CPPT_PUNCT &&
                            ...
                            /* Not `(` — the expansion result is NOT being
                             * called as a function.  Emit it normally,
                             * then push the peeked-aside tokens back into
                             * the main processing loop via pending_replay
                             * so that `#` at start-of-line (the next
                             * directive) is recognised correctly rather
                             * than emitted as raw text.                 */
                            emit_expanded_list(st, &expanded, macro_inv_loc, reader_filename(rd));
                            tl_free(&expanded);
                            rescanned = true;
                            /* Push the substantive non-`(` token */
                            PPTok _f = first;
                            _f.spell = first.spell;
                            first.spell = (StrBuf){0};
                            tl_prepend(&pending_replay, _f);
                            pptok_free(&first);
                            /* Reverse peek_ws, push to pending_replay so
                             * they replay in original left-to-right order. */
                            {
                                TokList ws_rev = {0};
                                for (TokNode *wn = peek_ws.head; wn; wn = wn->next) {
                                    PPTok cp = wn->tok;
                                    cp.spell = wn->tok.spell;
                                    wn->tok.spell = (StrBuf){0};
                                    tl_prepend(&ws_rev, cp);
                                }
                                tl_free(&peek_ws);
                                for (TokNode *rn = ws_rev.head; rn; rn = rn->next)
                                    tl_prepend(&pending_replay, rn->tok);
                                for (TokNode *rn = ws_rev.head; rn; rn = rn->next)
                                    rn->tok.spell = (StrBuf){0};
                                tl_free(&ws_rev);
                            }
                        }
                    }
...
void cpp_state_run_file(CppState *st, const char *filename) {
    install_builtins(st);
