...
> C++ Lite · 完整 C11 + struct 方法 + 泛型 + RAII · 转译到 C11 · 零运行时
...
**Sharp 是什么:** C + struct 内联方法 + C++ 风格的模板 + 确定性析构 + `@` 编译期内省。
...
struct Vec<T> {
    ...
    long len() const {           // const 方法:this 类型为 const Vec<T>*
        return this->len;
    }
...
|------------|----------|-------------------|
| 纯值类(`+`、`-`、`*`、`==`、一元 `-` 等) | `operator<op>(T this, ...)` 值传 | ✅ |
| 复合赋值(`+=`、`-=` 等) | `operator<op>(T* this, ...)` | ❌ |
...
方法、关联函数、析构函数、运算符全部定义在 `struct` 体内。struct 体既是数据布局的声明,也是该类型全部操作的命名空间。
...
析构函数 `~this()` 定义在 `struct` 体内,在变量离开作用域时自动调用。

### 规则

- 每个类型最多一个析构函数
- 析构函数无返回值、无参数(除隐式 `this`,类型为 `T*`)
...
**糖规则:** struct 体内的 `R operator<op>(Args)` 脱糖为 `R operator<op>(T this, Args)`(值传 `this`)。带 `const` 后缀时脱糖为 `R operator<op>(const T* this, Args)`。复合赋值类(`+=` 等)和 `[]` 的写入形式必须修改 `*this`,脱糖为 `R operator<op>(T* this, Args)`;该形式不能带 `const` 后缀。
...
Sharp **不允许**针对具体类型实参重新定义泛型 struct 的方法。类型差异统一用 `#if` 编译期分支在唯一的方法体内处理:

```sharp
// ❌ 不允许:无法为 Vec<int> 单独提供 clear 方法
// (语言不提供此机制)
...
对于 `Vec<int>`,生成的 C 代码中 `for` 循环根本不存在。对于 `Vec<Buffer>`,完整的析构循环被展开。两者均为零开销,逻辑全在一处。
...
**显式检查(推荐):** 在函数体开头用 `@static_assert` 提前到定义点检查,错误信息更清晰。

```sharp
T sum<T>(T* xs, long n) {
    @static_assert(@has_operator(T, +), "sum<T> requires T to support operator+");
    T acc = xs[0];
...
Sharp 没有独立的约束语法(`where` / concept / trait)。`@static_assert` 是唯一的显式约束机制。这个选择基于一个观察:泛型约束是库作者的工具,业务开发者几乎不需要写它。标准库内部用 `@static_assert` 保护接口,使用者直接用具体类型,感知不到这层存在。
...
两套系统视觉上彻底分离。`#if @has_destructor(T)` 这行代码中,`#if` 是预处理器条件,`@has_destructor(T)` 是编译器查询,扫一眼即可区分。

预处理器的 `#`(字符串化)和 `##`(拼接)只在 `#define` 替换列表内生效,与 `@` 无关。

### 约束即代码

`@` 原语返回普通布尔值/类型,可以任意组合在 `@static_assert` 中:

```sharp
// 要求 T 可加且尺寸 ≤ 16 字节
T fast_sum<T>(T* xs, long n) {
    @static_assert(@has_operator(T, +) && sizeof(T) <= 16,
        "fast_sum<T> requires T to support operator+ and sizeof(T) <= 16");
    ...
}

// 要求两个类型相同
void copy_same<A, B>(A* dst, B* src) {
    @static_assert(@is_same(A, B), "copy_same requires A and B to be the same type");
    *dst = *src;
}

// 要求 T 有 push 方法
void append_all<C, T>(C* container, T* items, long n) {
    @static_assert(@has_method(C, push), "append_all requires C to have a push method");
...
| `__SHARP_VERSION__` | `"0.10"` | Sharp 版本字符串 |
...
struct Vec<T> {
    ...
    void clear() {
        #if @has_destructor(T)             // T 有析构时生成循环,否则编译期擦除
            for (long i = 0; i < this->len; i++) ~T(&this->data[i]);
        #endif
        this->len = 0;
    }
...
T vec_sum<T>(const Vec<T>* v) {
    @static_assert(@has_operator(T, +), "vec_sum<T> requires T to support operator+");
    T acc = v->at(0);
    for (long i = 1; i < v->len(); i++) acc = acc + v->at(i);
    return acc;
}

// ---------- Vec3:struct 体内运算符 + 自由函数运算符 ----------
struct Vec3 {
    float x; float y; float z;
...
void main() {
    ...
    int s = vec_sum(&xs);                 // T 推断为 int
    xs.clear();                           // #if @has_destructor(int) → false → 直接清零
    ...
    Vec3 w = 2.0 * v;                     // 自由函数 operator*(float, Vec3)
    Vec3 u = v + w;                       // struct 体内 operator+
}
```
