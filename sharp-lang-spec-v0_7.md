...
3. **顶层 const 不参与函数类型匹配**(与 C 一致):`void f(const i32 x)` 与 `void f(i32 x)` 是同一函数类型。这条只对**顶层**参数成立;参数中的 `const T*` 仍然参与匹配。
4. **`const T x = expr;` 不是编译期常量**,与 C 一致。需要编译期整数常量时使用 `enum`。
5. **const 不能作为泛型类型实参的顶层限定。** `Vec<const i32>` 非法,因为 const 值类型不可赋值,与 C++ `std::vector<const T>` 规则一致。`Vec<const i32*>` 合法(const 在 pointee 上,非顶层)。
...
```sp
Vec<const u8*> strings;          // ✅ const 在 pointee 上,非顶层
const Vec<i32> v = make_vec();   // ✅ const 在变量上,非类型实参

Vec<const i32> v1;               // ❌ const 修饰值类型,顶层 const
Vec<i32* const> v2;              // ❌ const 修饰指针本身,顶层 const
```
...
```sp
impl<T> Vec<T> {
    usize len() const {           // const 方法:this 类型为 const Self*
        return this->len;
    }

    T at(usize i) const {          // const 方法
        return this->data[i];
    }
...
| 下标(`[]`)读取形式 | `operator[](const Self* this, usize)` | ✅ |
| 下标(`[]`)写入形式 | `operator[](Self* this, usize)` | ❌ |
...
const **不**导致泛型实例化膨胀:顶层 const 的类型实参被拒绝;`const Vec<T>`(const 在变量上)复用同一实例化;仅 `const T*`(const 在 pointee 上)产生独立实例化,这是有意为之——`Vec<const u8*>` 和 `Vec<u8*>` 确实需要不同的行为。
...
```c
// impl Buffer { Buffer alloc(usize n) { ... } }
Buffer Buffer__alloc(usize n) { ... }

// impl Buffer { u8 get(usize i) { ... } }
u8 Buffer__get(Buffer* this, usize i) { ... }
...
// a[i]:
operator_index__Vec3__usize(&a, i)
```
...
实例化:`Vec<i32>`, `Pair<i32, f32>`, `Vec<Pair<i32, f32>>`。
...
类型实参可从参数推断时允许省略:`swap(&x, &y)`。脱糖后等价于显式写 `swap<i32>`。
...
impl<T> Vec<T> {
    ...
    void grow() {
        usize new_cap = this->cap ? this->cap * 2 : 8;
...
impl    Vec<i32>        { void clear() { ... } }   // (B) 具体,比 A 严格更具体
...
Pair<i32, i32> p;   // 错误:A 和 B 都匹配,互不包含 → ambiguous impl
Pair<f32, i32> q;   // OK:仅 A 匹配
Pair<i32, f32> r;   // OK:仅 B 匹配
```

**消歧:** 处理 ambiguity 案例须显式提供完全具体的 impl:

```sharp
impl Pair<i32, i32>     { ... }   // (C) 严格最具体
Pair<i32, i32> p;                  // OK:选 C
```
...
```sharp
T sum<T>(T* xs, usize n) {
    T acc = xs[0];
    for (usize i = 1; i < n; i++) acc = acc + xs[i];   // 要求 T 有 operator+
    return acc;
}
...
T sum<T>(T* xs, usize n) {
    ...
    T acc = xs[0];
    for (usize i = 1; i < n; i++) acc = acc + xs[i];
    return acc;
}
```
...
```sharp
T sum<T>(T* xs, usize n) where #has_operator(T, +) {
    T acc = xs[0];
    for (usize i = 1; i < n; i++) acc = acc + xs[i];
    return acc;
}
...
```sharp
// lib.sp:实例化 Vec<i32> 到此翻译单元
impl Vec<i32>;

// user.sp:声明 Vec<i32> 在别处实例化,本 TU 不生成代码
extern impl Vec<i32>;
```
...
```
Vec<i32>                   → Vec__i32
Pair<i32, f32>             → Pair__i32__f32
Vec<Pair<i32, f32>>        → Vec__Pair__i32__f32
Vec<i32>::push             → Vec__i32__push
operator+ (Vec3, Vec3)     → operator_add__Vec3__Vec3
operator[] (Vec<i32>, usize) → operator_index__Vec__i32__usize
```
...
// 要求 T 有 push 方法
void append_all<C, T>(C* container, T* items, usize n)
    where #has_method(C, push)
{
    for (usize i = 0; i < n; i++) container->push(items[i]);
}
```
...
```sharp
void destroy_n<T>(T* xs, usize n) {
    #if #has_destructor(T)
        for (usize i = 0; i < n; i++) ~T(&xs[i]);
    #endif
}
```
...
impl<T> Vec<T> {
    ...
    usize len() const {                     // const 方法:可用于 const 接收者
        return this->len;
    }
    ...
    T operator[](usize i) const {           // const 读取形式
        return this->data[i];
    }
...
T vec_sum<T>(const Vec<T>* v) where #has_operator(T, +) {
    ...
    for (usize i = 1; i < v->len(); i++) acc = acc + v->at(i);
    return acc;
}
...
impl Vec<i32> {
    void clear() {                      // (A) 比泛型 impl 更具体
        this->len = 0;                  // i32 无析构,直接清零
    }
}

impl<T> Vec<T> {
    void clear() {                      // (B) 泛型兜底
        for (usize i = 0; i < this->len; i++) ~T(&this->data[i]);
        this->len = 0;
    }
}
...
// ---------- 使用 ----------
void main() {
    Vec<i32> xs = Vec<i32>.new();
    xs.push(1); xs.push(2); xs.push(3);
    i32 s = vec_sum(&xs);                    // T 推断为 i32;Vec<i32>* → const Vec<i32>*
    xs.clear();                              // 选 (A),严格最具体

    const Vec<i32>* ro = &xs;
    usize n = ro->len();                     // OK:len() 是 const 方法
    // ro->push(4);                          // 编译错误:push 非 const
    printf("sum = %d, len = %zu\n", s, n);
    // xs 离开作用域 → ~Vec<i32>() → ~i32 被擦除 → free(xs.data)
