...
void ast_node_free(AstNode *node) {
    ...
    case AST_FILE:
        ...
        astvec_deep_free(&node->u.file.sys_decls);
        for (size_t _i = 0; _i < node->u.file.nuser_includes; _i++)
            free(node->u.file.user_includes[_i]);
        free(node->u.file.user_includes);
        break;
    case AST_EXTERN_INST:
        ast_node_free(node->u.extern_inst.type);
        break;

    case AST_GCC_VERBATIM:
        free(node->u.gcc_verbatim.text);
        break;
    case AST_STRUCT_DEF:
    ...
    case AST_FUNC_DEF:
        ...
        free(node->u.func_def.gcc_attrs);  /* */
        free(node->u.func_def.trailing_gcc_attrs); /* C8 */
        free(node->u.func_def.struct_name);  /* out-of-line method */
        break;
    case AST_TYPEDEF_DECL:
        free(node->u.typedef_decl.alias);
        free(node->u.typedef_decl.gcc_attrs);
        /* C8: if target_unowned, the target is shared with file->decls
         * and will be freed there — don't double-free it. */
        if (!node->u.typedef_decl.target_unowned)
    ...
    /* ── Type nodes ──────────────────────────────────────────────────── */
    case AST_TYPE_NAME:
        free(node->u.type_name.name);
        /* display_name is heap-allocated via cpp_xstrndup when set;
         * the const qualifier is intentional (the string is immutable
    ...
    case AST_TYPEOF_TYPE:
        free(node->u.typeof_type.verbatim);
        ast_node_free(node->u.typeof_type.expr);
        break;
    ...
    case AST_FIELD_DECL:
        ...
        free(node->u.field_decl.gcc_attrs);
        break;
    case AST_PARAM_DECL:
        ast_node_free(node->u.param_decl.type);
        free(node->u.param_decl.name);
        free(node->u.param_decl.gcc_attrs);
        break;
    ...
    case AST_INT_LIT:   free(node->u.int_lit.orig_text);         break;
    case AST_FLOAT_LIT: free(node->u.float_lit.orig_text);       break;
    case AST_STRING_LIT:/* text is borrowed — do NOT free */     break;
    case AST_CHAR_LIT:  free(node->u.char_lit.orig_text);        break;
    ...
    case AST_CAST:
        ...
        break;
    case AST_PAREN:   /* C8 */
        ast_node_free(node->u.paren.inner);
        break;
    case AST_SIZEOF:
        ast_node_free(node->u.sizeof_.operand);
        free((void*)node->u.sizeof_.alignof_spelling);
        break;
    case AST_STRUCT_LIT:
    ...
    case AST_GENERIC_EXPR:
        ast_node_free(node->u.generic_expr.controlling);
        astvec_deep_free(&node->u.generic_expr.associations);
        break;
    case AST_GENERIC_ASSOC:
        ast_node_free(node->u.generic_assoc.type_name);
        ast_node_free(node->u.generic_assoc.value);
        break;

    case AST_GENERIC_CALL:   /* Phase G */
        free(node->u.generic_call.name);
        astvec_deep_free(&node->u.generic_call.type_args);
        astvec_deep_free(&node->u.generic_call.call_args);
        break;
 ...
 * sem_type/sem_scope to copy — the clone is a pre-sema AST), so this is straight-
...
AstNode *ast_clone_type(const AstNode *node) {
    ...
    /* Fast path: AST_TYPE_NAME is the most common type node (int, char, void,
     * struct Tag, typedef alias …).  It carries only a name string and two
     * bool flags; copy the whole node with memcpy then duplicate the name.
     * This avoids the calloc-then-zero in ast_node_new for this hot path.   */
    if (node->kind == AST_TYPE_NAME) {
        AstNode *n = malloc(sizeof *n);
        if (!n) { perror("sharp-fe ast_clone_type"); abort(); }
        *n = *node;   /* bitwise copy: kind, loc, fmt, is_struct_tag, is_enum_tag */
        n->sem_type  = NULL;   /* clone is pre-sema */
        n->sem_scope = NULL;
        n->u.type_name.name =
            cpp_xstrdup(node->u.type_name.name ? node->u.type_name.name : "");
        n->u.type_name.display_name = node->u.type_name.display_name
            ? cpp_xstrdup(node->u.type_name.display_name) : NULL;
        return n;
    }
...
const char *ast_kind_name(AstKind k) {
    static const char *names[] = {
        ...
        "CAST", "PAREN", "SIZEOF", "STRUCT_LIT",
        "INIT_LIST", "DESIGNATED_INIT", "COMPOUND_LIT",
        "AT_INTRINSIC",
        "STMT_EXPR",    /* GCC ({ ... }) */
        "GENERIC_EXPR",  /* C11 _Generic(...) */
        "GENERIC_ASSOC", /* _Generic type:val   */
        "GENERIC_CALL",  /* Phase G:  func<int>(args)     */
    };
    /* Compile-time guard: the names array MUST have exactly AST_COUNT entries.
     * Adding a new AST kind to ast.h without adding its name here will
     * produce a compile error. */
    _Static_assert(sizeof(names)/sizeof(names[0]) == AST_COUNT,
                   "ast_kind_name array must have exactly AST_COUNT entries");
...
void ast_print(const AstNode *node, int depth, FILE *fp) {
    ...
    case AST_PAREN:   /* C8 */
        fprintf(fp, "(PAREN\n");
        ast_print(node->u.paren.inner, depth+1, fp);
        indent_print(depth, fp); fprintf(fp, ")\n");              break;
    case AST_SIZEOF:
        fprintf(fp, "(SIZEOF %s\n",
    ...
    case AST_GENERIC_CALL:  /* Phase G */
        fprintf(fp, "(GENERIC_CALL %s\n", node->u.generic_call.name);
        print_vec(&node->u.generic_call.type_args, "type_args", depth+1, fp);
        print_vec(&node->u.generic_call.call_args, "call_args", depth+1, fp);
        indent_print(depth, fp); fprintf(fp, ")\n");              break;
