            ...
            if (vty && vty->kind == AST_TYPE_NAME && vty->u.type_name.name) {
                ...
                for (size_t _k = 0; _k < ctx->local_block_stmts->len; _k++) {
                    ...
                    if (_dd && _dd->kind == AST_STRUCT_DEF &&
                        (_dd->u.struct_def.from_inline_var ||
                         strncmp(_dd->u.struct_def.name ? _dd->u.struct_def.name : "", "__anon_", 7) == 0) &&
                        _dd->u.struct_def.name &&
                        strcmp(_dd->u.struct_def.name, vnm) == 0) {
                        vsd = _dd; break;
                    }
                }
                if (!vsd && ctx->file_ast) {
                ...
                if (vsd && vsd->u.struct_def.fields.len > 0) {
                    ...
                    for (size_t _j = 0; _j < vsd->u.struct_def.fields.len; _j++) {
                        ...
                        if (_fj && _fj->kind == AST_FIELD_DECL && _fj->u.field_decl.is_comma_cont) continue;
                        cg_puts(ctx, "  ");
                            ...
                            for (size_t _k = 0; _k < ctx->local_block_stmts->len; _k++) {
                                ...
                                if (_dd && _dd->kind == AST_STRUCT_DEF &&
                                    (_dd->u.struct_def.from_inline_var ||
                                     strncmp(_dd->u.struct_def.name ? _dd->u.struct_def.name : "", "__anon_", 7) == 0) &&
                                    _dd->u.struct_def.name &&
                                    strcmp(_dd->u.struct_def.name, _vnm) == 0) {
                                    _vsd = _dd; break;
                                }
                            }
                        }
                        if (!_vsd && ctx->file_ast) {
                            for (size_t _k = 0; _k < ctx->file_ast->u.file.decls.len; _k++) {
                                const AstNode *_dd = ctx->file_ast->u.file.decls.data[_k];
                                if (_dd && _dd->kind == AST_STRUCT_DEF &&
                                    (_dd->u.struct_def.from_inline_var ||
...
static bool cg_emit_field_maybe_inline(CgCtx *ctx,
    ...
    for (size_t _k = 0; _k < ctx->file_ast->u.file.decls.len; _k++) {
        ...
        for (size_t _j = 0; _j < _td->u.struct_def.fields.len; _j++) {
            ...
            if (_f && _f->kind == AST_FIELD_DECL && _f->u.field_decl.is_comma_cont) continue;
            cg_puts(ctx, "        ");
...
static void cg_file(CgCtx *ctx, const AstNode *file) {
    ...
    /* Standard library includes needed by common Sharp codegen patterns */
    cg_puts(ctx, "#include <stdlib.h>\n");
    ...
    for (size_t i = 0; i < file->u.file.decls.len; i++) {
        ...
        if (!decl_is_user(ctx, d)) {
            /* System-header typedefs that are simple type aliases
             * (e.g. typedef int __kernel_rwf_t;) are needed by user
             * code.  Skip struct/enum-bodied typedefs — they would
             * conflict with system headers already included. */
            AstNode *uw2 = cg_unwrap_decl(d);
            if (!uw2 || uw2->kind != AST_TYPEDEF_DECL) continue;
            AstNode *tgt = uw2->u.typedef_decl.target;
            if (!tgt) continue;
            /* Unwrap qualifiers (const, volatile, _Atomic) to find the base type name */
            AstNode *base = tgt;
            while (base->kind == AST_TYPE_CONST ||
                   base->kind == AST_TYPE_VOLATILE ||
                   base->kind == AST_TYPE_ATOMIC) {
                base = base->u.type_volatile.base;
            }
            /* Only emit typedefs whose base is a known type name
             * (int, long, unsigned int, …) or another named alias.
             * Skip anonymous struct/enum names generated by the parser. */
            if (base->kind != AST_TYPE_NAME) continue;
            const char *name = base->u.type_name.display_name;
            if (!name) name = base->u.type_name.name;
            if (!name) continue;
            /* Reject synthetic anonymous struct/enum refs
             * (__anon_struct_…, __anon_union_…, __anon_enum_…)
             * and ordinary struct tags (starts with an uppercase letter). */
            if (strncmp(name, "__anon_", 7) == 0) continue;
