 ...
 * fast switch-based dispatch.  Sharp keywords ('defer', 'operator',
...
static const KwEntry kw_table[] = {
    ...
    /* c */ { "case",             STOK_CASE          },
            ...
            { "const",            STOK_CONST         },
            { "constexpr",        STOK_CONSTEXPR     },  /* C23: compile-time constant */
    ...
    /* _ */ { "_Alignas",         STOK__ALIGNAS      },
            ...
            /* GCC __thread = _Thread_local alias */
            { "__thread",         STOK__THREAD_LOCAL },
            ...
            { "typeof",           STOK_TYPEOF        },
            { "__typeof",         STOK_TYPEOF        },
            { "__typeof__",       STOK_TYPEOF        },
            /* GCC __auto_type = type-deduced auto */
            { "__auto_type",      STOK_AUTO          },
            /* GCC __extension__ — no-warn prefix for statement-exprs.
             * Used extensively in glibc's assert.h and stdatomic.h. */
            { "__extension__",    STOK__EXTENSION__  },
            /* GCC __alignof / __alignof__ (without the standard
            ...
            { "__asm",            STOK_ASM           },
            { "__asm__",          STOK_ASM           },
            /* MSVC attribute extension: __declspec(keyword)
             * Syntax uses single parens, unlike __attribute__((...)). */
            { "__declspec",       STOK_DECLSPEC      },
            /* Clang nullability annotations — used extensively in macOS /
             * iOS system headers (e.g. lua.h, dispatch.h).  Promoted to
             * dedicated token kinds so the parser can preserve them in
             * C output without mistaking them for variable names. */
            { "_Nullable",        STOK__NULLABLE     },
            { "_Nonnull",         STOK__NONNULL      },
            { "_Null_unspecified", STOK__NULL_UNSPEC },
};
static const size_t kw_count = sizeof kw_table / sizeof kw_table[0];

/* =========================================================================
 * Token classification (keyword / punctuation / literal kind)
 * ====================================================================== */
...
static SharpTokKind keyword_kind(const char *text, size_t len) {
...
static size_t int_suffix_len(const char *text, size_t len) {
    ...
    if (len - s == 1 && s >= 2) {
        char c = text[s-1];
        if (c == 'l' || c == 'L') {
            /* We have  ... ll  before the 'u' we consumed.
             * Reset and try u-first. */
            s = len;
            if (s >= 2 && (text[s-1]=='u'||text[s-1]=='U')) s--;
            if (s >= 3 && (text[s-2]=='l'||text[s-2]=='L') && text[s-1]==text[s-2]) s -= 2;
            else if (s >= 2 && (text[s-1]=='l'||text[s-1]=='L')) s--;
...
/* =========================================================================
 * Literal parsing (integers, floats, characters)
 * ====================================================================== */
...
SharpTok *lex_run(const CppTok *tokens, size_t ntokens,
    ...
    for (size_t i = 0; i < ntokens; i++) {
         ...
         * non-ISO construct that follows.  Pass it through as a plain IDENT. */
        ...
        st.text       = (ct.text && ct.len > 0)
                        ...
                        : cpp_xstrndup("", 0);
        st.len        = ct.len;
        /* strdup loc.file — it is an interned string owned by CppCtx
         * (freed by cpp_ctx_free) but AstNode.loc.file must outlive the
         * context (e.g. when an included file's CppCtx is freed before the
         * main file's cg pass reads the loc). */
        st.loc        = ct.loc;
        if (ct.loc.file)
            st.loc.file = cpp_xstrndup(ct.loc.file, strlen(ct.loc.file));
        st.from_macro = ct.from_macro;

        switch (ct.kind) {
        case CPPT_IDENT:
            /* Phase 1: keyword promotion. */
            st.kind = keyword_kind(ct.text, ct.len);
            break;
        ...
        /* String concatenation (ISO C translation phase 6) is intentionally
         * NOT done here. We are a source-to-source transpiler; the downstream
         * C compiler will perform phase 6 concatenation. Doing it here would
         * break token-identical output: gcc -E stops at phase 4 and preserves
         * `"foo" "bar"` as two separate tokens. */
...
void lex_free(SharpTok *toks) {
    if (!toks) return;
    /* Free each token's text copy and loc.file copy (both allocated by
     * cpp_xstrndup in lex_run so SharpTok is independent of CppCtx). */
    for (size_t i = 0; toks[i].kind != STOK_EOF || i == 0; i++) {
        free((char *)toks[i].text);
        free((char *)toks[i].loc.file);  /* C8: strdup'd in lex_run */
