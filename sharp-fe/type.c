...
#include "type.h"
#include "lex.h"
#include "scope.h"   /* scope_lookup_struct_tag */
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
Type *ty_struct_type(TyStore *ts, const char *name,
    ...
    Type candidate = { .kind = TY_STRUCT,
                       .u.struct_ = { iname, false, args, nargs, decl } };
    for (size_t i = 0; i < ts->compound_len; i++) {
        if (ty_compound_eq(ts->compound[i], &candidate)) {
            ...
            /* Scope-shadowing fix: if BOTH the existing and new decl have
             * struct bodies AND they point to different AST nodes (i.e. a
             * block-scoped `struct T { ... }` shadowing a file-scoped one),
             * do NOT reuse the interned type — fall through to allocate a
             * new Type* with the inner decl.  Without this, both `struct T`
             * definitions share one Type* whose decl field stays at the
             * first (outer) body, causing field-access sema to see the wrong
             * members for the inner struct. */
            if (nd_has_body && od_has_body && od != decl)
                break; /* fall through to allocate fresh Type* */
...
bool ty_is_float(const Type *t) {
    ...
    return t->kind == TY_FLOAT || t->kind == TY_DOUBLE || t->kind == TY_LONGDOUBLE;
}
...
/* recursively check for TY_ERROR anywhere in the type chain.
 * Used by cg.c to detect types with local-struct bases (e.g.
 * TY_PTR(TY_CONST(TY_ERROR)) for `const EncName *pEnc`). */
bool ty_has_error(const Type *t) {
    if (!t) return false;
    switch (t->kind) {
...
bool ty_is_scalar(const Type *t) {
    /* __typeof__(...) verbatim types are opaque — we can't
     * know the real type.  Treat as scalar so that while/if/ternary
     * conditions using atomic types don't generate spurious errors. */
...
Type *ty_from_name(TyStore *ts, const char *name) {
    ...
    if (!strcmp(name,"__signed__ __int128"))         return ty_longlong(ts);
    if (!strcmp(name,"unsigned __int128"))           return ty_ulonglong(ts);
    ...
    if (!strcmp(name,"__float128") ||
        !strcmp(name,"_Float128"))                   return ty_longdouble(ts, "__float128");
    if (!strcmp(name,"_Float128x"))                  return ty_longdouble(ts, "_Float128x");
    if (!strcmp(name,"__float80"))                   return ty_longdouble(ts, "__float80");
    if (!strcmp(name,"__cfloat128"))                 return ty_longdouble(ts, "__cfloat128");
    if (!strcmp(name,"_Float64x"))                   return ty_longdouble(ts, "_Float64x");
    /* Explicit-width floats from C TS 18661 / GCC */
    ...
    if (!strcmp(name,"_Decimal128"))                 return ty_longdouble(ts, "long double");
    return NULL;
}
...
static int64_t eval_array_size(TyStore *ts, const AstNode *expr,
    ...
    case AST_SIZEOF:
        ...
        if (expr->u.sizeof_.operand->kind == AST_TYPE_NAME) {
            ...
            case TY_LONGDOUBLE: return sizeof(long double);
            default: return -1;
            }
        }
        return -1;
    ...
    case AST_IDENT: {
        /* array size may be an enum constant or a `const int`
    ...
    case AST_TERNARY: {
        /* `MAX(a,b)` macro expands to `(a)>(b) ? (a) : (b)`.
...
/* =========================================================================
 * typeof-root-fix: evaluate an expression AST (produced by the mini-parser
 * inside the typeof type-specifier handler) to infer its concrete C type.
 * Returns NULL if the expression cannot be evaluated; the caller then
 * falls back to the opaque typeof struct.
 * ====================================================================== */

static Type *typeof_eval_expr(TyStore *ts, AstNode *expr, Scope *scope) {
    if (!expr) return NULL;

    switch (expr->kind) {
    case AST_INT_LIT:
        return expr->u.int_lit.is_unsigned ? ty_uint(ts) : ty_int(ts);
    ...
    case AST_STRING_LIT:
        return ty_ptr(ts, ty_char(ts));

    case AST_IDENT: {
        if (!expr->u.ident.name) return NULL;
        Symbol *s = scope_lookup(scope, expr->u.ident.name);
        if (s && s->kind == SYM_VAR && s->decl && s->decl->sem_type)
            return s->decl->sem_type;
        return NULL;
    }

    case AST_PAREN:
        return typeof_eval_expr(ts, expr->u.paren.inner, scope);

    case AST_UNARY: {
        Type *ot = typeof_eval_expr(ts, expr->u.unary.operand, scope);
        if (!ot) return NULL;
        switch (expr->u.unary.op) {
        case STOK_MINUS:
        case STOK_PLUS:
            return ty_is_arithmetic(ot) ? ot : NULL;
        case STOK_BANG:
            return ty_int(ts);
        case STOK_TILDE:
            return ty_is_integer(ot) ? ot : NULL;
        case STOK_STAR: {
            if (!ot || ot->kind != TY_PTR) return NULL;
            return ot->u.ptr.base;
        }
        default:
            return NULL;
        }
    ...
    case AST_BINOP: {
        Type *lt = typeof_eval_expr(ts, expr->u.binop.lhs, scope);
        Type *rt = typeof_eval_expr(ts, expr->u.binop.rhs, scope);
        if (!lt || !rt) return NULL;
        switch (expr->u.binop.op) {
        case STOK_PLUS: case STOK_MINUS: case STOK_STAR:
        case STOK_SLASH: case STOK_PERCENT:
        case STOK_AMP: case STOK_PIPE: case STOK_CARET:
        case STOK_LTLT: case STOK_GTGT:
            if (!ty_is_arithmetic(lt) || !ty_is_arithmetic(rt))
                return NULL;
            return ty_is_float(lt) || ty_is_float(rt)
                   ? ty_double(ts) : ty_int(ts);
        case STOK_LT: case STOK_GT: case STOK_LTEQ: case STOK_GTEQ:
        case STOK_EQEQ: case STOK_BANGEQ:
        case STOK_AMPAMP: case STOK_PIPEPIPE:
            return ty_int(ts);
        case STOK_COMMA:
            return rt;
        default:
    ...
    case AST_CAST:
        if (expr->u.cast.type)
            return ty_from_ast(ts, expr->u.cast.type, scope, NULL);
        return NULL;
...
Type *ty_from_ast(TyStore *ts, const AstNode *node,
    ...
    case AST_STRUCT_DEF: {
        /* struct/union AST used as type specifier. */
        const char *sname = node->u.struct_def.name;
        if (!sname || !sname[0]) return ty_error(ts);
        /* transparent union (__attribute__((transparent_union))).
         * Used by glibc's __CONST_SOCKADDR_ARG and __SOCKADDR_ARG.  Emit
         * the first valid field's type so function declarations use the
         * concrete pointer type (e.g. `const struct sockaddr *`) rather
         * than the anonymous union — which cc can't match to sockaddr casts. */
        if (node->u.struct_def.is_transparent_union &&
            node->u.struct_def.fields.len > 0) {
        ...
        if (!has_body && scope) {
            Symbol *sym = scope_lookup_struct_tag(scope, sname);
            if (sym && sym->decl && sym->decl->kind == AST_STRUCT_DEF)
                return ty_struct_type(ts, sname, NULL, 0, sym->decl);
        }
        return ty_struct_type(ts, sname, NULL, 0, (AstNode *)node);
    }

    case AST_TYPEOF_TYPE:
        /* typeof-root-fix: try to infer the concrete C type from the
         * inner expression AST.  Fall back to opaque struct if the
         * expression cannot be evaluated (e.g. complex expressions or
         * forward-referenced identifiers). */
        if (node->u.typeof_type.expr) {
            Type *inferred = typeof_eval_expr(ts,
                (AstNode *)node->u.typeof_type.expr, scope);
            if (inferred) return inferred;
        }
        return ty_struct_type(ts, node->u.typeof_type.verbatim,
                              NULL, 0, (AstNode *)node);
    ...
    case AST_TYPE_NAME: {
        ...
        if (prim) return prim;
        /* verbatim typeof(...) strings */
        if (strncmp(name, "__typeof__(", 11) == 0)
            return ty_struct_type(ts, name, NULL, 0, NULL);
        /* when the type name came from an explicit `struct Tag`
         * or `union Tag` usage (is_struct_tag == true), look up the struct
         * definition specifically via scope_lookup_struct_tag.  This ensures
         * that `struct h1_tunnel_state ts` resolves to the STRUCT body even
         * when a same-named typedef (e.g. for an enum) is also in scope and
         * would be found first by scope_lookup_type. */
        if (node->u.type_name.is_struct_tag && scope) {
        ...
        if (sym->kind == SYM_TYPE) {
            ...
            if (sym->decl && sym->decl->kind == AST_STRUCT_DEF &&
                sym->decl->u.struct_def.is_transparent_union &&
            ...
            if (sym->decl && sym->decl->kind == AST_STRUCT_DEF &&
                ...
                Type *ret = ty_struct_type(ts, name, gargs, ng, sym->decl);
                free(gargs);
                return ret;
            }
            return ty_struct_type(ts, name,
    ...
    case AST_TYPE_FUNC: {
        ...
        for (size_t i = 0; i < np; i++) {
            ...
            /* skip the vararg sentinel (is_vararg=true, type=void).
             * Without this, ty_func gets a TY_VOID param for `f(T, ...)` which
             * cg emits as `f(T, void)` — invalid C. */
            if (pi && pi->kind == AST_PARAM_DECL && pi->u.param_decl.is_vararg) {
                has_vararg = true;
                continue;
            }
...
Type *ty_subst(TyStore *ts, Type *t,
    ...
    case TY_STRUCT:
        if (t->u.struct_.nargs > 0) {
            ...
            if (!na) abort();
            for (size_t i = 0; i < t->u.struct_.nargs; i++)
                na[i] = ty_subst(ts, t->u.struct_.args[i], pnames, pvals, np);
