...
本阶段双线推进：(1) **zlib 1.3.1** 11 个 .c 文件（共 ~22K 行）作为继 Lua、
cJSON、picol、stb_image 之后的第五个真实 C 集成目标，端到端字节对齐；
(2) **sqlite-amalgamation 3.46.1**（258K 行单文件）作为 surface
validation 目标——暴露 6 类深 bug 全已修，但因两个 GCC builtin
(`__builtin_va_arg`、`__atomic_*`) 暂时不能完整端到端，留作将来阶段。

**项目状态一行总结**：R3 完成。三道门 203/203；c-superset 66/67；五个真实
目标（Lua + cJSON + picol + stb_image + zlib）端到端均通过且字节对齐。

**最重要的 4 件事给下一位**：

1. **`build_zlib_with_sharpfe.sh` 是 R3 的核心产物**（继 S5/R1/R2 的同套
   范式）。验收方法：cc-only 跑出 baseline，再跑完整管线 (cpp+fe+cc+link
   +run)，diff baseline。一致 = 通过。zlib 11/11 全部 .c 一遍过 +
   PNG round-trip 输出 byte-for-byte。

2. **R3 修了 8 类深 bug**（全带 `Phase R3:` 注释），不要回退：
   6 类源自 sqlite 实战 + 2 类是 cg 改动的 fallout：

   - **坑 FE-37（已修）— `IDENT (` 不被 is_type_start 识别**：sqlite 的
     `sqlite3_syscall_ptr (*xGetSystemCall)(...)` 形态——typedef-name 作
     函数指针字段返回类型——is_type_start IDENT 后只接受 `IDENT/*/(/)/<`
     而漏 `(`。修法：加 `IDENT (` 分支，gated by `td_has_n`（与 `IDENT )`
     cast 检测同模式）。Probe `p61_typedef_fnptr_field.c`。

   - **坑 FE-38（已修）— `__attribute__((fallthrough));` 单独成 stmt
     失败**：parse_stmt 把它路由到 declaration 路径，parse_decl_specifiers
     吃完 attribute 后期待 type-specifier 报"missing type specifier"。
     修法：parse_stmt 早识别 STOK_ATTRIBUTE + `;`，吃掉返回 NULL（block
     walker 过滤）。Probe `p63_attribute_stmt.c`。

   - **坑 FE-39（已修，双侧）— ISO C99 匿名 bit-field `int :32;`**：
     parse 报"field declaration is missing a name"；即使穿过 parse，多个
     `int :N;` 在 build_struct 因都用 name=`""` 互相冲突。修法：parse 在
     declarator 无名 + 下一 token 是 `:` 时合成空名；scope.c build_struct
     跳过空名 SYM_FIELD 注册。Probe `p62_anon_bitfield.c`。

   - **坑 FE-40（已修，双侧）— 后置 `T const *` 在 cast / parse_type
     失败**：parse_type apply_suffix 只接受指针上的后置 const，base type
     的后置 const 没处理；同时 is_type_start 漏了 `IDENT const` 模式。
     修法：apply_suffix 入口加后置 const/volatile/restrict 吸收循环；
     is_type_start 加 `IDENT const/volatile/restrict` typedef 守卫。
     Probe `p64_postfix_const.c`。

   - **坑 FE-41（已修，双侧）— 函数指针抽象 cast `(RetT (*)(args))expr`**：
     parse_type 不接受抽象 `(*)(args)` declarator；cg_type 对 TY_PTR(TY_FUNC)
     发射 `func *`（丢了参数表）。修法：parse_type apply_suffix 加 `(`+`*`
     检测，构造 TYPE_PTR(TYPE_FUNC) 树；cg_type TY_PTR 检测 PTR-to-FUNC
     时发射抽象 `ret (*)(args)` 形式。Probe `p66_abstract_fnptr_cast.c`。

   - **坑 FE-42（已修）— ISO C99 §6.9.2 tentative definition**：sqlite 的
     `static u32 sqlite3WhereTrace;` ... `static u32 sqlite3WhereTrace = 0;`
     模式（暂定定义+真定义）。pre-R3 只接受 extern 跨步，不接受 static 或
     无 storage 跨步。修法：scope_define 扩展规则——后到的 decl 若有 init
     而 existing 无 init，无视 storage class 一律 promote。Probe
     `p67_tentative_def.c`。

   - **坑 FE-43（cg fallout，已修）— 函数返回函数指针的嵌套 declarator**：
     FE-41 的 cg_type 改动让 `int (*)(int)` 在 cast/sizeof 正确，但破坏了
     Lua 的 `extern int (*lua_atpanic(...))(lua_State *);` 函数原型——
     变成 flat 形式 `int (*)(...) lua_atpanic(...)` 不合法。修法：cg_func
     检测 ret_t peel 到 TY_FUNC 时，发射嵌套 declarator
     `inner_ret (*name(outer_args))(inner_args)`。Probe `p68_fn_returning_fnptr.c`。

   - **坑 FE-44（cg fallout，已修）— fn-ptr 数组的嵌套 declarator**：
     Lua 的 `static const lua_CFunction searchers[] = {...};` 同类问题——
     原本 cg_decl array 调 cg_type(elem) 拼 ` name[]`，TY_PTR(TY_FUNC) 元素
     现在发射 `int (*)(int)`，结果 `int (*)(int) const handlers[]` 双重
     不合法（flat fnptr + const 错位）。修法：cg_decl array 分支加 PTR-to-FUNC
     检测（剥可选 const），用嵌套 `int (* const name[])(args)` 形式。
     Probe `p69_fnptr_array.c`。

3. **sqlite 留了 3 个 deferred 限制**——都是 GCC builtin/扩展，单独成阶段：
   - `__builtin_va_arg(ap, T)` —— T 是 *类型* 参数，需要专门 parser+sema
   - `__atomic_load_n` / `__atomic_store_n` —— GCC builtin atomics
   - `__int128` —— 128 位整数，目前用 `-D__int128=long` 宏 workaround（仅
     在 unused typedef 中出现，无副作用）

   做完这三个能解锁 sqlite + 大量生产 C 代码（zstd、libpng 等）。

4. **R3 完全没碰 sharp-cpp**，第二个连续阶段做到这点。sharp-fe 持续吃下
   越来越大的真实代码而不需要 cpp 改动——这是 cpp/fe 边界稳定的强信号。

**bug 数演化**：
- S5 = 2（Lua 30K）
- R1 = 4（cJSON 2K）
- R2 = 4（picol 0 + stb 4，10.8K）
- R3 = **8**（zlib 0 + sqlite 6 + cg fallout 2，22K + 258K）

R3 是 fix 量最大的一阶段。**bug-per-target 不单调下降**——sqlite 表面比
之前所有目标加起来还大，自然撞出更多模式。但 zlib 的 0-bug（与 picol 在
R2 的 0-bug 对应）持续印证：当某个表面被 prior fixes 覆盖后，新目标会一
遍过。

**Phase R4 候选（推荐下一动作）**：

- **GCC builtins 专项** ——把上述 3 个 deferred 限制（`__builtin_va_arg` +
  `__atomic_*` + statement-expression）做完，解锁 sqlite 和大量 production
  C 代码。这是单一最高价值的下一步。
- **第六个真实目标** ——`tinycc`（结构上自相似 = 编译 C 编译器自身）、
  `lz4`、`mcpp`、`nuklear` 任一。
- **D1 dogfooding** ——sharp-fe 模块开始用 Sharp 方言重写。

**基线**：69 probes + 40 integration + 25 import + 69 decls = **203 项,
全 0 fail**。c_superset 66/67。Lua + cJSON + picol + stb_image + zlib
端到端均通过，全部字节对齐。

— Phase R3 接力人，于 Phase R3 收尾时
