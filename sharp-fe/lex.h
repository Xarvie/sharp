...
typedef enum {
    ...
    STOK__EXTENSION__, /* '__extension__' — GCC no-warn prefix.  Silently
                       * consumed before expressions and declarations so
                       * assert.h / stdatomic.h statement-expressions are
                       * parsed correctly.  CG preserves it in C mode. */
    ...
    STOK_DECLSPEC,   /* __declspec — MSVC attribute specifier introducer.
                      * Syntax: __declspec(keyword) with single parens.
                      * Sharp treats these like __attribute__: parses and
                      * emits verbatim for the C compiler.                */

    /* ── Clang nullability annotations (macOS / iOS) ──────────────────── */
    STOK__NULLABLE,    /* _Nullable   — Clang nullability annotation       */
    STOK__NONNULL,     /* _Nonnull    — Clang nullability annotation       */
    STOK__NULL_UNSPEC, /* _Null_unspecified — Clang nullability annotation */
 ...
 *
 * Sharp-specific keywords (`defer`, `operator`, `this`) are always promoted
 * to their `STOK_*` token kinds.  C11 keywords are promoted unconditionally.
 *
