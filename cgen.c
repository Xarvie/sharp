...
static Type* type_of_int_literal(G* g)   { return type_prim(g->arena, TY_I32);  }
static Type* type_of_float_literal(G* g) { return type_prim(g->arena, TY_F64);  }
static Type* type_of_bool_literal(G* g)  { return type_prim(g->arena, TY_BOOL); }
static Type* type_of_char_literal(G* g)  { return type_prim(g->arena, TY_U8); }
...
static void emit_expr(G* g, Node* e) {
    ...
    switch (e->kind) {
        ...
        case ND_PRINTLN: {
            ...
            if (vt) {
                switch (ty_kind(vt)) {
                    case TY_I8: case TY_I16: case TY_I32: case TY_ISIZE:
                        fmt = "%d"; break;
                    case TY_I64:
                        fmt = "%lld"; break;
                    case TY_U8: case TY_U16: case TY_U32: case TY_USIZE:
                        fmt = "%u"; break;
                    case TY_U64:
                        fmt = "%llu"; break;
                    ...
                    case TY_BOOL:
                        ...
                        break;
                    case TY_PTR:
                        /* u8* → treat as string (print char arrays). */
                        if (ty_base(vt) && ty_kind(ty_base(vt)) == TY_U8)
                            fmt = "%s";
                        else
                            fmt = "%p";
                        break;
