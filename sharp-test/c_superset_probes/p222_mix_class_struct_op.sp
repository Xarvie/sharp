/* p222 — free-function operator mixing class and struct
 *
 * Verifies that free-function operators work correctly when mixing
 * class (with typedef) and struct types in the same operator:
 *   - class + struct  (cross-type)
 *   - struct + class  (cross-type reverse)
 *   - class == struct (cross-type comparison)
 */

/* --- class types (auto-typedef) --- */
class VecC {
    float x; float y;
}

/* --- struct types (no typedef) --- */
struct VecS { float x; float y; }

/* === class + struct => class (cross-type) === */
...
/* === struct + class => class (cross-type reverse) === */
