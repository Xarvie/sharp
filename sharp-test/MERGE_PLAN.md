...
│   │   ├── sizeof.ref.i
│   │   ├── compound_literal.ce
│   │   └── compound_literal.ref.i
│   ├── runtime/                   # 运行时验证 (若干 → 5)
│   │   ├── generic_monomorph.ce
│   │   ├── generic_monomorph.ref.i
│   │   ├── multi_inst.ce
│   │   ├── multi_inst.ref.i
│   │   ├── struct_layout.ce
│   │   ├── extmethod_chain.ce
│   │   └── extmethod_chain.ref.i
│   ├── stdlib/                    # 标准库集成
│   │   ├── cjson_test.ce
│   │   └── raw_string_test.ce
│   └── static_var.ce              # 静态变量
...
│   ├── bugs/                      # Bug 回归 (5 个保持不变)
│   │   ├── bug001_stddef_preamble.ce
│   │   ├── bug002_const_auto.ce
│   │   ├── bug003_ptr_return_method.ce
│   │   ├── bug004_typedef_return.ce
│   │   └── bug005_local_func_decl.ce
...
#### 1.1 struct.ce — 合并 6 个文件

**来源**: p101, p103, p104, p105, p115, p107

**内容规划**:
```sharp
// 来源: p101_generic_struct.ce, p103_generic_nested.ce,
//       p104_generic_ptr.ce, p105_generic_method.ce,
//       p115_generic_struct_lit.ce, p107_generic_multitype.ce
...
#### 1.2 selfref.ce — 合并 2 个文件

**来源**: p106, p238_selfref

**内容规划**:
```sharp
// 来源: p106_generic_selfref.ce, p238_generic_selfref.ce
...
#### 1.3 function.ce — 合并 4 个文件

**来源**: p102, p117, p118, p119

#### 1.4 nested.ce — 合并 3 个文件

**来源**: p233, p153, p152

#### 1.5 typedef.ce — 合并 3 个文件

**来源**: p114, p224, p149

#### 1.6 operator.ce — 合并 2 个文件
...
#### 2.1 return.ce — 合并 3 个文件

**来源**: p108, p257, p266(return 部分)

#### 2.2 loop.ce — 合并 2 个文件

**来源**: p109, p266(loop 部分)

#### 2.3 complex.ce — 保留 p155
...
#### 3.1 basic.ce — 合并 2 个文件

**来源**: p110, p154(基础部分)

#### 3.2 index.ce — 保留 p112

**来源**: p112(独立功能)

#### 3.3 unary.ce — 合并 2 个文件

**来源**: p113, p249

#### 3.4 chain.ce — 合并 2 个文件

**来源**: p239, p246

#### 3.5 precedence.ce — 合并 2 个文件

**来源**: p247, p248

#### 3.6 extension.ce — 合并 2 个文件
...
- basic.ce: p111 + p157(基础)
- complex.ce: p157(边界) + p231

#### 4.2 const: 9 → 5

- method.ce: p141 + p147 + p148
- pointer.ce: p226
- overload.ce: p156 + p244
- generic.ce: p142 + p146
- volatile.ce: p271 + p278

#### 4.3 modules: 11 → 5 + 8 .he

- basic.ce + basic.he: p158
- cross_ref.ce + cross_ref.he: p159 + p261
- stdlib.ce: p161
- generic_inst.ce + generic_inst.he: p163
- (p162, p164 合并入上述)
...
- has_operator.ce: p50
- has_method.ce: p250 + p251 + p252
- static_assert.ce: p253 + p220
- v013_intrinsics.ce: p219

#### 5.2 extensions: 9 → 5

- basic.ce: p223 + p228
- method_call.ce: p240
- dispatch.ce: p241 + p242 + p243
- composition.ce: p245
- runtime.ce: p264 + p265
...
- to_cstr.ce: p218
- std_include.ce: p221 + p227
- (pc00220 移至 compatibility)

#### 6.2 controlflow: 6 → 4

- switch.ce: p254
- loops.ce: p255 + p256 + p258
- goto.ce: p259
- string_literal.ce: p274
...
- typedef_chain.ce: p269
- enum.ce: p272
- struct_array.ce: p273
- sizeof.ce: p275
- compound_literal.ce: p276

#### 7.2 runtime: 新增分类

- generic_monomorph.ce: p268
- multi_inst.ce: p270
- struct_layout.ce: p264
- extmethod_chain.ce: p265
...
# 移动所有被合并的文件
mv c_superset_probes/p101*.ce LEGACY/
mv c_superset_probes/p102*.ce LEGACY/
# ... (按合并计划移动)
...
| .ref.i 文件同步问题 | 中 | 中 | 合并后重新生成 .ref.i |
...
### Phase 1 检查项
- [ ] 创建 unit/generics/ 目录
- [ ] 编写 struct.ce 并生成 struct.ref.i
- [ ] 编写 selfref.ce 并生成 selfref.ref.i
- [ ] 编写 function.ce 并生成 function.ref.i
- [ ] 编写 nested.ce 并生成 nested.ref.i
- [ ] 编写 typedef.ce 并生成 typedef.ref.i
- [ ] 编写 operator.ce 并生成 operator.ref.i
- [ ] 运行 `run_regression.py` 验证泛型测试
- [ ] 确认通过率 100%

### Phase 2 检查项
- [ ] 创建 unit/defer/ 目录
- [ ] 编写 return.ce 并生成 return.ref.i
- [ ] 编写 loop.ce 并生成 loop.ref.i
- [ ] 复制 complex.ce (p155)
- [ ] 运行回归测试验证
- [ ] 确认通过率 100%
