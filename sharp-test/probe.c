 ...
 * PURPOSE
 * -------
 * Before writing any front-end code, probes map every core feature of
 ...
 *
 * Two probe kinds:
 *   CPP probe     — calls cpp_run_buf (sharp_mode=true by default).
...
static CppResult cpp_sharp(const char *src) {
    ...
    CppResult r = cpp_run_buf(ctx, src, strlen(src), "<probe>");
    cpp_ctx_free(ctx);
    return r;
}
...
static void p1_6(void) {
    /* '@' is always recognised as CPPT_AT_INTRINSIC (sharp_mode=true) */
    CppCtx *ctx = cpp_ctx_new();
    cpp_emit_linemarkers(ctx, false);
    CppResult r = cpp_run_buf(ctx, "@foo", 4, "<p>");
    cpp_ctx_free(ctx);
    bool has_at = count_kind(&r, CPPT_AT_INTRINSIC) == 1;
    cpp_result_free(&r);
    if (has_at) ok("P1.6", "'@' is always CPPT_AT_INTRINSIC (unified)");
    else        iss("P1.6", "'@' token not recognised as AT_INTRINSIC");
}
...
static void p1_8(void) {
    ...
    cpp_result_free(&r);
    if (rshift)
        ok("P1.8", "'>>' is a single token — parse_generic_args splits via pending_close (implemented)");
    else if (gt_count >= 2)
...
static void p6_4(void) {
    return; /* removed: @has_operator is not designed for #if context */
}
/*
static void p6_4_old(void) {
    CppResult r = cpp_sharp(
        "#if @has_operator(T, +)\nint x = 1;\n#endif\n");
...
/* =========================================================================
 * §14 — auto type deduction (Phase 1 addendum)
 * ====================================================================== */

/* P14.1  'auto x = expr;' — 'auto' arrives as CPPT_IDENT, same as C11 auto.
...
static void p14_1(void) {
    ...
    cpp_result_free(&r);
    if (found && no_err)
        ok("P14.1", "'auto' type deduction: arrives as CPPT_IDENT (same as C11 auto)");
    else
        iss("P14.1", "Unexpected: 'auto' not IDENT or cpp error");
}

/* P14.2  'const auto x = expr;' — both 'const' and 'auto' are CPPT_IDENT. */
static void p14_2(void) {
    ...
    if (found_auto && found_const && no_err)
        ok("P14.2", "const auto: both 'const' and 'auto' are CPPT_IDENT");
    else
        iss("P14.2", "Unexpected: const/auto token or cpp error");
}

/* DECISION probe for §auto */
...
int main(void) {
    ...
    printf("SS14 auto type deduction\n");
    p14_1(); p14_2(); p14_d1();
    printf("\n");
