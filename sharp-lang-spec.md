...
Sharp 内置一个完整的 C11 预处理器（`cpp/` 模块），在词法分析之前运行。它与 C 预处理器完全兼容。Sharp 使用 `#include` 引入其他 Sharp 源文件（`.ce` / `.he`），预处理器在所有源文件中统一工作，宏行为与 C 完全一致。
...
| 宏                   | 值                   | 说明                     |
| ------------------- | ------------------- | ---------------------- |
| `__FILE__`          | `"path/to/file.ce"` | 当前源文件路径                |
| `__LINE__`          | 整数                  | 当前源文件行号                |
...
| `__SHARP__`         | `1`                 | Sharp 语言模式（仅 `.ce` 文件） |
