...
void cgen_c(Node* prog, SymTable* st, FILE* out) {
    ...
    for (int i = 0; i < st->nexterns; i++) {
        ...
        /* Skip externs that come from system/SDK headers — they are already
         * provided by the standard headers we #include at the top. */
        if (e->decl && is_system_header_path(e->decl->source_file)) {
            continue;
        }
